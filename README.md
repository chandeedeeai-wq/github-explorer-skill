...
- GitHub repo 主页面是客户端渲染页面，`web_fetch` 不适合直接抓取 `github.com/{org}/{repo}` 页面壳子。
- 获取 README、Repo 元数据、Issues、Commits 时，优先使用当前主机已登录的 `gh` CLI 和 `gh api`。
- 推荐先检查认证状态：

```bash
/home/leo/.local/bin/gh auth status
```

- 只有当运行环境已经安全注入 `GH_TOKEN` / `GITHUB_TOKEN`，且 `gh` 不可用时，才考虑环境变量方案。
- 不要把 PAT 明文直接拼进 `curl -H "Authorization: ..."` 命令；这会把凭据暴露给 shell history、进程列表或日志。
- 不要在日志、命令回显或分析报告里输出 token、Authorization header 或其他敏感认证信息。
...
3. **抓取降级** — GitHub repo 数据优先走 `gh api`；其他网页在 `web_fetch` 失败时升级为 `content-extract`（MinerU），保证内容质量
4. **拒绝空泛** — 社区声量必须引用具体内容，不允许"评价很高"这种概括
5. **并行采集** — 多源同时抓取 + 多查询并行（`--queries`），提高效率
6. **凭据安全** — 优先复用 `gh` 本地登录态，不把 GitHub PAT 直接写进命令行
